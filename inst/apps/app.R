...
ui <- function() {
  fluidPage(
    ...
    titlePanel(div(
      ...
      div(
        h4(
          "A user friendly dashboard for refining omics identifiers and cross-referencing mappings across different biological datasources."
        ),
            ...
            sidebarLayout(
              ...
              sidebarPanel(
                ...
                radioButtons(
                  ...
                  HTML(
                    paste(
                      "Mapping type&nbsp;<i class='fas fa-info-circle info-icon' data-toggle='tooltip' title='",
                      tooltips$description[tooltips$tooltip == "Mapping type"],
                      "'></i> :"
                    )
                  ),
                  inline = TRUE,
                ...
                fileInput(
                  ...
                  HTML(
                    paste(
                      "Upload identifiers file&nbsp;<i class='fas fa-info-circle info-icon' data-toggle='tooltip' title='",
                      tooltips$description[tooltips$tooltip == "Identifier file"],
                      "'></i> :"
                    )
                  ),
                ...
                textAreaInput(
                  ...
                  HTML(
                    paste(
                      "or insert identifier(s) here&nbsp;<i class='fas fa-info-circle info-icon' data-toggle='tooltip' title='",
                      tooltips$description[tooltips$tooltip == "Identifiers"],
                      "'></i> :"
                    )
                  ),
                  value = "",
              ...
              mainPanel(
                ...
                div(
                  plotOutput("sec2pri_piechart_results", height = "200px"),
                  class = "my-plot"
                ),
    ...
    div(
      ...
      div(style = "flex: 0;"),
      div(# imageOutput("bridgeDb_logo_wide", height = "70px"),
        style = "background-color: white; text-align: center; padding: 10px;"),
      div(p(
        "licensed under the ",
        a("Apache License, version 2.0", href = "https://www.apache.org/licenses/LICENSE-2.0")
      ),
...
server <- function(input, output, session) {
  ...
  output$sec2pri_download <- downloadHandler(
    filename = function() {
      paste0(
        "OmicsFixID_mapping_",
        gsub(" ", "_", input$sec2priDataSource),
        ".",
        input$sec2pri_download_format
      )
    },
